...
if(class(ref_sce)== "SingleCellExperiment"){
...
colData(ref_sce) <- colData(ref_sce)[, cols_to_keep, drop = FALSE]
  ...
  cols_to_keep <- names(ref_sce@meta.data)[!grepl("\\.", ref_sce@meta.data)]
  ref_sce@meta.data <- ref_sce@meta.data[,cols_to_keep, drop = FALSE]
}
...
ref_adata <- SCE2AnnData(ref_sce)
  ...
  for (i in names(spe@meta.data)) {
    if (class(spe[[i]]) == "DFrame") {
      spe[[i]] <- NULL
    }
  }
  h5ad_path <- here("Datasets/seurat_query.h5ad")
  if (file.exists(h5ad_path)) {
    message(paste(
      "h5ad query file already exists at:",
      h5ad_path,
      ". Skipping conversion."
    ))
    return(FALSE)
  } else {
    SaveH5Seurat(spe, filename = here("Datasets/seurat_query.h5Seurat"))
    Convert(here("Datasets/seurat_query.h5Seurat"), dest = "h5ad")
  }
  
  adata_path <- here("Datasets/seurat_query.h5ad")
...
if(class(ref_sce)=="Seurat") {
  ...
  h5ad_path <- here("Datasets/seurat_ref.h5ad")
  if (file.exists(h5ad_path)) {
    message(paste(
      "h5ad ref file already exists at:",
      h5ad_path,
      ". Skipping conversion."
    ))
    return(FALSE)
  } else {
    SaveH5Seurat(ref_sce, filename = here("Datasets/seurat_ref.h5Seurat"))
    Convert(here("Datasets/seurat_ref.h5Seurat"), dest = "h5ad")
  }
  adata_path <- here("Datasets/seurat_ref.h5ad")
  ref_adata <- anndata$read_h5ad(adata_path)
}
